...
extern void web_config_init(void);
...
VOID InfLoadDefaultParam(VOID)
{
	...
	// wifi config
	sprintf((char *)cfg->WiFiCfg.Ssid, "Nufront_Serial%02x%02x", PermanentAddress[4], PermanentAddress[5]); 
	...
	cfg->bDhcpServer = 1;
	cfg->bDnsServer = 1;
	strcpy((char *)cfg->DnsName, "nufront.com");
