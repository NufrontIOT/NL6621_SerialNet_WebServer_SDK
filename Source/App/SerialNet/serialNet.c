...
VOID LoadUserParam(VOID)
{
	...
	strcpy((char *)SysParam.DnsName, "nufront.com");
...
int TestSerialToWifi(void)
{
	...
	/* Web server process task thread */
	sys_thread_new("WebServerThread", WebServerThread, NULL, NST_TEST_APP_TASK_STK_SIZE*2, prioUser++); //8

	/* UDP broadcast receive task thread. */
	sys_thread_new("BCTRecvThread", BCTRecvThread, NULL, NST_TEST_APP_TASK_STK_SIZE, prioUser++);//9
